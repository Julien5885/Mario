...
<style>
    ...
    .btn-cancel {
        background: linear-gradient(135deg, rgb(233, 47, 47), rgb(242, 14, 14));
        border: 2px solid rgb(244, 10, 10); 
        color: white;
        @apply rounded-full;
        box-shadow: 0 4px 6px rgba(0, 0, 0, 0.2);
    }
...
<div class="container mx-auto px-4 py-8">
    ...
    <!-- Formulaire d'ajout -->
    <form method="POST" action="{{ route('inventory.store') }}" class="bg-white p-6 rounded shadow-md space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 gap-4">
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}"
                       class="border border-gray-300 rounded-full w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label for="description" class="block font-bold text-gray-800 mb-2">Description :</label>
                <textarea id="description" name="description"
                          class="border border-gray-300 rounded-full w-full py-2 px-3 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description') }}</textarea>
            </div>
            <div>
        ...
        <div class="flex items-center justify-between mt-6">
            <button type="submit" class="btn-custom btn-submit rounded-full px-6">
                Enregistrer
            </button>
            <a href="{{ route('inventory') }}" class="btn-custom btn-cancel rounded-full px-6">
                Annuler
            </a>
        </div>
    </form>
